const SCENE={ alley:'img/0.png', intro:'img/1.png', normal:'img/2.png', portrait:'img/3.png', atari:'img/4atari.png', sikatanai:'img/5sikatanai.png', ending:'img/6en.png' };

// ── 音量設定 ──────────────────────────────────────────
const VOL_KEY = 'BAR_HYOKAI_VOLUME';
const VOLUME = (() => {
  try { return JSON.parse(localStorage.getItem(VOL_KEY)) || {bgm:0.6, jingle:0.7}; }
  catch(e){ return {bgm:0.6, jingle:0.7}; }
})();
function saveVolume(){ localStorage.setItem(VOL_KEY, JSON.stringify(VOLUME)); }
...
const BGM = {
  ...
  play(name, loop=true){
    ...
    this.stop();
    const audio = new Audio(`bgm/${name}`);
    audio.loop = loop;
    audio.volume = VOLUME.bgm;
    audio.play().catch(()=>{});
    this._current = audio;
    this._name = name;
  },
  ...
  fadeOut(ms=800){
    ...
    const timer = setInterval(()=>{
      ...
      else { audio.pause(); clearInterval(timer); }
    }, 50);
    this._current = null;
    this._name = null;
  }
...
const ENDING_CARDS = [
  {id:'allA', title:'全A', text:'結果より過程。AいしAいされたようだ。', image:'img/7A.png', unlocked: s => s.turnLabels && s.turnLabels.length && s.turnLabels.every(x=>'A'===x)},
  {id:'allB', title:'全B', text:'結果より過程。BBッときたようだ。', image:'img/8B.png', unlocked: s => s.turnLabels && s.turnLabels.length && s.turnLabels.every(x=>'B'===x)},
  {id:'allC', title:'全C', text:'結果より過程。これ以外はCらないようだ。', image:'img/9C.png', unlocked: s => s.turnLabels && s.turnLabels.length && s.turnLabels.every(x=>'C'===x)},
  {id:'score10', title:'10問正解', text:'記憶を取り戻した。そしてこの場を見ていたい。', image:'img/10.png', unlocked: s => s.score===10},
  {id:'score6to9', title:'6〜9問正解', text:'うやむやな時間。たまには面白いこともあるもんだ。', image:'img/11.png', unlocked: s => s.score>=6 && s.score<=9},
  {id:'score0to5', title:'0〜5問正解', text:'とほほ…高いお金を払ってその場所を後にした。', image:'img/12.png', unlocked: s => s.score>=0 && s.score<=5}
];
...
function showFinal(){
  clearChoices(); setBg('normal');
  BGM.play('bgm_think.mp3');
  const q=QS[G.qi]; const num=Math.max(1,6-G.cTurns); let idxs=[q.correct];
...
function pickFinal(ri){
  ...
  clearChoices();
  updateHud();
  playJingle(ok ? 'jingle_correct.mp3' : 'jingle_wrong.mp3');
  setTimeout(()=>BGM.play('bgm_bar.mp3'), 1500);
...
function showEnding(){
  ...
  setBg('ending');
  BGM.fadeOut();
  setTimeout(()=>BGM.play('bgm_ending.mp3', false), 900);
...
function openModal(type){
  ...
  if(type==='menu'){
    ...
    body.innerHTML='<p>現在：'+esc(now)+'　正解数：'+G.score+'/10</p>'
      +'<p style="margin-top:16px"><label>BGM音量　<input type="range" min="0" max="1" step="0.05" value="'+VOLUME.bgm+'" oninput="VOLUME.bgm=+this.value; if(BGM._current) BGM._current.volume=VOLUME.bgm; saveVolume()"></label></p>'
      +'<p style="margin-top:8px"><label>効果音音量　<input type="range" min="0" max="1" step="0.05" value="'+VOLUME.jingle+'" oninput="VOLUME.jingle=+this.value; saveVolume()"></label></p>'
      +'<p style="margin-top:18px"><button class="primary" onclick="saveGame()">セーブ</button> <button class="primary" onclick="loadGame()"'+(hasSavedGame()?'':' disabled')+'>ロード</button> <button class="primary" onclick="openModal(\'cards\')">カードを見る</button></p>'
      +'<p style="margin-top:12px"><button class="primary" onclick="location.reload()">タイトルへ戻る</button></p>';
  }
  if(type==='cards'){
    applyAchievements();
    title.textContent='エンディングカード';
